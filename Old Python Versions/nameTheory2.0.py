...
with open("C:/Users/aloes/OneDrive/Code/Name Theory/names.json") as f:
    file = json.load(f)
...
#brings up a dropdown menu of all available names under a letter for user choice
...
def genAdviceButtonPressed():
    ...
    #ignores all keys to display the last key (aka the general advice key)
    for key, val in file[nameStr[0]].items():
        if "advice" in key:
            #expands the window if the general advice would run off the minimum borders
            mainWindow.minsize(1000,600) if len(val) > 500 else mainWindow.minsize(1000,500)
            genAdviceLabel.config(text = "-----\n" + val)
...
mainWindow = tk.Tk()
mainWindow.wm_title("Allison's Name Theory, version 2.0.2")
